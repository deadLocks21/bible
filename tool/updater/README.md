...
Installateur / launcher / **auto-updater silencieux** pour Bible desktop
...
| Plateforme | Asset |
|---|---|
| Windows | `bible-updater-windows.exe` (binaire nu) |
| Linux | `bible-updater-linux` (binaire nu) |
| macOS | `bible-installer-macos-<version>.zip` (« Bible Installer.app ») |
...
La fenêtre est **rendue par l'app Bible elle-même** (`current/bible
...
<root>/                      racine choisie à l'install
  versions/<v>/              contenu d'une version (bible.exe + dll + data/ sous
                             Windows, squashfs-root/ extrait sous Linux,
                             bible.app sous macOS)
  current      ->  versions/<v>   jonction (Windows) / symlink (Linux, macOS)
  updater/bible-updater[.exe]   le binaire relocalisé (cible stable des raccourcis)
...
Racine par défaut : `%LOCALAPPDATA%\Bible` (Windows),
`~/.local/share/Bible` (Linux),
`~/Library/Application Support/Bible` (macOS).
...
L'updater est un binaire **distinct** de `bible.exe`, et il met à jour
...
chaque lancement. Repli sur `Bible.AppImage` si l'extraction échoue, ou pour
...
- **macOS** : un bundle lanceur `~/Applications/Bible.app` dont l'exécutable
...
cd tool/updater
dart pub get
dart compile exe bin/bible_updater.dart -o bible-updater
```
...
`bible-macos-installer-*` (il matcherait la regex de l'app, qui s'installerait

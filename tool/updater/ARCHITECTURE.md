...
Conçu pour Bible mais pensé pour être **répliqué** : la fin du document liste
...
```
<root>/                         %LOCALAPPDATA%\Bible (Win) | ~/.local/share/Bible (Linux)
                                | ~/Library/Application Support/Bible (macOS)
  versions/
    1.10.1/                     contenu d'une version (exe + dll + data/, AppImage
    1.10.2/                     extraite, ou bible.app)
  current        ->  versions/1.10.2     jonction (Windows) / symlink (Linux, macOS)
  updater/
    bible-updater(.exe)       le binaire updater, relocalisé ici (cible des raccourcis)
...
pointe **toujours** sur `updater/bible-updater`, **jamais** sur un exe
...
`Terminal=false` ; sous macOS le lanceur `~/Applications/Bible.app` est un
...
- Assets attendus par l'updater (à matcher par regex / nom) :
  - App Windows : `bible-windows-<v>.zip` (contenu du dossier `Release/`).
  - App macOS : `bible-macos-<v>.zip` (`.app` Developer ID notarisé, zippé
    par `ditto`).
  - App Linux : `bible-linux-<v>-x86_64.AppImage`.
  - Updater Windows/Linux : `bible-updater-windows.exe`,
    `bible-updater-linux` — noms **exacts et non versionnés** (les installs
    existantes les cherchent ainsi pour s'auto-mettre à jour).
  - Installateur macOS : `bible-installer-macos-<v>.zip`. Le préfixe
    `bible-installer-macos-` est obligatoire : renommé
...
- `lib/shortcuts.dart` — `.lnk` + `launch.vbs` (Windows) / `.desktop` (Linux) /
  bundle lanceur `~/Applications/Bible.app` (macOS).
...
2. **Adapter `lib/layout.dart`** :
   ...
   - `defaultRoot()` / `pointerFile()` : remplacer « Bible » par le nom de l'app.

...
`.pkg` n'est plus produit. Les testeurs qui utilisaient Bible macOS depuis
TestFlight **ne recevront plus de build** — il faut leur faire installer
`Bible Installer.app` une fois (cf. §6). Les builds déjà distribués continuent
...
commit feat/fix sur main (GitLab)
  └─ semantic-release : bump pubspec.yaml + tag vX.Y.Z (GitLab)
       └─ mirror-to-github : pousse le tag vers deadLocks21/bible (GitHub)
...
**Conséquence : les secrets se configurent sur GitHub** (`deadLocks21/bible`),
...
Puis, sur **GitHub → `deadLocks21/bible` → Settings → Secrets and variables →
...
2. **`codesign --verify` ou la notarisation rejette** — récupère le détail :
   ...
   liste `find` si un helper particulier remonte. Bible embarque `media_kit`
...
5. **`Bible Installer.app` : l'icône rebondit dans le Dock puis plus rien**
   ...
   ```bash
   codesign -d --entitlements - --xml "Bible Installer.app" | grep unsigned-executable
   # et, dans ~/Library/Logs/DiagnosticReports/bible-installer-*.ips :
...
6. **La 1re install marche, mais `~/Applications/Bible.app` ne fait rien**
   ...
   **scellant le hash de son `Info.plist`** ; copié hors de `Contents/MacOS/`
   ...
   ```bash
   codesign --verify --strict "$HOME/Library/Application Support/Bible/updater/bible-updater"
   ...
   D'où la **seconde copie** du binaire dans `Contents/Resources/bible-updater`,
...
`bible-installer-macos-<v>.zip` — un **`Bible Installer.app`** notarisé
**et staplé**.

Côté utilisateur, **première fois seulement** :

1. Télécharger `bible-installer-macos-….zip` et le dézipper (double-clic).
2. **Double-cliquer `Bible Installer.app`.**
...
- Il installe l'app sous `~/Library/Application Support/Bible`, crée un
  lanceur **`~/Applications/Bible.app`**, puis démarre l'app. On peut ensuite
  jeter `Bible Installer.app`.

**Ensuite**, l'utilisateur lance toujours via `~/Applications/Bible.app`
...
- **Pas de Mac App Store** : macOS n'est distribué qu'en direct (plus de `.pkg`,
  ...
  `bible-installer-macos-<v>.zip` (le `.app` installateur) ; l'app elle-même
  (`bible-macos-<v>.zip`) est récupérée par l'updater.
...
- [ ] `Bible Installer.app` (dans `bible-installer-macos-<v>.zip`) récupéré

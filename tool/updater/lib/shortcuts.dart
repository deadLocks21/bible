...
/// stables (`launch.vbs`, `current\bible.exe`), et les réécrire ressusciterait
...
foreach ($dir in @([Environment]::GetFolderPath('Desktop'), [Environment]::GetFolderPath('Programs'))) {
  $lnk = $shell.CreateShortcut((Join-Path $dir 'Bible.lnk'))
  $lnk.TargetPath = 'wscript.exe'
  $lnk.Arguments = '"' + $vbs + '"'
  $lnk.IconLocation = $iconExe + ',0'
  $lnk.WorkingDirectory = $workDir
  $lnk.Description = 'Bible'
  $lnk.Save()
}
''';
  ...
  try {
    final r = Process.runSync('powershell', [
      ...
      psFile.path,
      layout.launchVbs,
      layout.appExecutable, // current\bible.exe -> icône
      layout.root,
    ]);
    if (r.exitCode != 0) {
...
void _createMacosLauncher(Layout layout, Log log) {
  ...
  final bundle = Directory(p.join(home, 'Applications', 'Bible.app'));
  ...
  final launcher = File(p.join(macosDir.path, 'Bible'));
  ...
  // Bundle id distinct de l'app (`fr.dtfh.bible`) pour ne pas se marcher
...
<plist version="1.0">
<dict>
	<key>CFBundleName</key><string>Bible</string>
	<key>CFBundleDisplayName</key><string>Bible</string>
	<key>CFBundleIdentifier</key><string>fr.dtfh.bible.launcher</string>
	<key>CFBundleExecutable</key><string>Bible</string>
...
const _applicationId = 'fr.dtfh.bible';
...
''';
  final file = File(p.join(appsDir.path, 'bible.desktop'));
  file.writeAsStringSync(desktop);
  Process.runSync('chmod', ['+x', file.path]);

  // Petit symlink CLI pratique : `bible` dans ~/.local/bin.
  final binDir = Directory(p.join(home, '.local', 'bin'));
  if (binDir.existsSync()) {
    final link = Link(p.join(binDir.path, 'bible'));

...
class Installer {
  ...
  /// Installe Bible dans la racine de [layout]. [launch] démarre l'app à la
  ...
  Future<void> install({bool launch = true}) async {
    ...
    try {
      ...
      log('Installation terminée : Bible ${release.version}');
      if (launch) launchApp();
    } finally {
  ...
  void launchApp() {
    ...
    if (Platform.isMacOS) {
      final app = p.join(layout.currentLink, 'Bible.app');
...
const _relocatableName = 'bible-updater';
...
String _relocatableBinary(String executable) {
  ...
  final candidate = p.join(p.dirname(macosDir), 'Resources', _relocatableName);
...
Future<Process?> _startInstallFeedback() async {
  ...
  try {
    final proc = await Process.start('osascript', [
      '-e',
      'display dialog "Installation de Bible en cours… (quelques secondes)" '
          'buttons {"OK"} default button 1 giving up after 120 '
          'with title "Bible" with icon note',
    ]);

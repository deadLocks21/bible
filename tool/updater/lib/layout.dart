...
/// Décrit l'arborescence d'une installation Bible enracinée en [root] :
///
/// ```
/// <root>/
...
class Layout {
  ...
  String get updaterExe => p.join(
    updaterDir,
    Platform.isWindows ? 'bible-updater.exe' : 'bible-updater',
  );
  ...
  String get appExecutable {
    if (Platform.isWindows) return p.join(currentLink, 'bible.exe');
    if (Platform.isMacOS) {
      // On vise le binaire INTERNE du bundle (et non `open bible.app`) : ça
      ...
      // `bible.app` / `bible` suivent PRODUCT_NAME (macos/.../AppInfo.xcconfig).
      return p.join(currentLink, 'Bible.app', 'Contents', 'MacOS', 'Bible');
    }
    final appRun = p.join(currentLink, 'squashfs-root', 'AppRun');
    if (File(appRun).existsSync()) return appRun;
    return p.join(currentLink, 'Bible.AppImage');
  }
  ...
  String get appIcon {
    final png = p.join(currentLink, 'squashfs-root', 'bible.png');
  ...
  /// Fichier-pointeur hors-racine, qui mémorise OÙ Bible est installé.
  ...
  static String pointerFile() {
    if (Platform.isWindows) {
      ...
      return p.join(appData, 'Bible', 'install.path');
    }
    if (Platform.isMacOS) {
    ...
    return p.join(base, 'bible', 'install.path');
  }

...
8. **Utiliser uniquement des imports absolus** : tous les imports doivent commencer par `package:bible/` (jamais de chemins relatifs avec `../`)
...
7. **Utiliser des imports relatifs** : ne jamais utiliser `../` dans les imports (toujours préférer `package:bible/`)
...
Trois familles, à l'image de `test/` :

- `test/builders/` — constructeurs d'objets de domaine, avec des valeurs par défaut plausibles : un test ne précise que les champs qui le concernent ;
- `test/unit/` — domaine, cas d'usage, et les `dio.*.repository.dart` face à un `Dio` bouchonné (`test/unit/helpers/stub_dio.dart`), là où le format d'échange avec l'API se vérifie ;
- `test/functionnals/` — les écrans, montés depuis `AuthGate` comme au démarrage, avec des implémentations de test des **ports du domaine** à la place des repositories HTTP (`test/functionnals/utils/`). C'est le bénéfice direct de l'architecture : aucun faux serveur, aucune implémentation en mémoire embarquée dans `lib/`.

Le lecteur YouTube s'appuie sur une WebView, indisponible en test : les tests fonctionnels utilisent des lectures sans vidéo.
